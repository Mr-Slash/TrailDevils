...
public enum ImageDownloader {
	...
	/**
	 * Searches the cache if the Drawable already exists in the HashMap. If yes, then it will be set on the view.
	 * Else a default image is set to the view and a thread starts do download the image from the web in the background.
	 * 
	...
	/**
	 * Creates a handler in the UI thread and sets the images when the background thread 
	 * notifies his download as completed. Pay attention on the THREAD_POOL_SIZE and regulate
	 * it if required. 
	 * 
	 * @param url the url to the image
	 * @param imageView the view to set the image
	...
	/**
	 * Saves a drawable in the cache with a soft reference. 
	 * If systems gets out of memory, those references will be deleted quickly.
	 * Additional it monitors the list size of drawables and reduces it if necessary
	 * to avoid an overload of drawables.
	 * 
	 * @param url the url to the image
	 * @param drawable the image to save
	 */
